!(function(){
    ...
    window.addEventListener('scroll', function() {
      if (!ticking) {
        requestAnimationFrame(function() {
          n.classList.toggle('on', window.scrollY > 60);
          ticking = false;
        });
        ticking = true;
      }
  ...
  if (b && l) {
    b.addEventListener('click', function() {
      ...
      document.body.style.overflow = l.classList.contains('open') ? 'hidden' : '';
    });
  ...
  if (c.length) {
    var co = new IntersectionObserver(function(e) {
      e.forEach(function(en) {
        if (en.isIntersecting) {
          var el = en.target,
              t = parseInt(el.getAttribute('data-target')),
    ...
    c.forEach(function(el) { co.observe(el); });
  }

  document.querySelectorAll('.faq-i').forEach(function(it) {
    it.querySelector('.faq-q').addEventListener('click', function() {
      var w = it.classList.contains('open');
      document.querySelectorAll('.faq-i.open').forEach(function(o) { o.classList.remove('open'); });
